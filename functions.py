...
def datime_for_brdcfile(year, month, day, hour, minute, sec):
    return str(str(year)[2:] + '%02i' % int(month) + '%02i' % int(day) + ' %02i' % int(hour) + ':%02i' % int(
        minute) + ':%02i' % int(sec))
...
def creat_nav_file(list_check_data, f_log, path=os.getcwd(), n_sys=1, year='2022', day_year='001', brdc_datetime='',
                   date_ver='010122'):
    if list_check_data is not None:
        ...
        s1 = "     3.02            N:GNSS NAV DATA    R: {0}RINEX VERSION / TYPE\n" \
             ...
             "                                                            END OF HEADER\n".format(
            sys_alphafullnumeric_dict.get(n_sys), date_ver, brdc_datetime)
        f.write(s1)
